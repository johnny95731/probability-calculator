...
const vuetifyInputKeydownWrapper = (originStep, targetStep, ref2, range) => {
  const { min, max } = range ?? {};
  ...
  return (e) => {
    if (e.key === "ArrowUp") {
      if (!(ref2.value > +max - targetStep))
        ref2.value += unit;
      else
        ref2.value = max;
    } else if (e.key === "ArrowDown") {
      if (!(ref2.value < +min + targetStep))
        ref2.value -= unit;
      else
        ref2.value = min;
    }
...
const mergeObjects = (...objs) => {
  if (!(objs == null ? void 0 : objs.length)) return {};
  const result = {};
  for (; objs.length > 0; ) {
...
function toCSSLength(str) {
  if (isNullish(str)) return;
  if (!isNaN(+str) || // Numberish
  // can be transform to a number.
  !isContainingUnit(str) && !isNaN(parseFloat(str))) return `${parseFloat(str)}px`;
  return str;
}
function isContainingUnit(str) {
  return /p[xtc]|in|Q|%|[cem]m|v[hw]/.test(str);
}
const EPSILON = 1e-13;
const PLACE = 7;
const SQRT2PI = Math.sqrt(2 * Math.PI);
const SQRTPI_2 = Math.sqrt(Math.PI / 2);
const round = (num, place = 0) => Math.round(10 ** place * num) / 10 ** place;
...
function sortArr(arr, callback) {
  callback ?? (callback = (a, b) => a - b);
  const copy = JSON.parse(JSON.stringify(unref(arr)));
  return copy.sort(callback);
}
function arange(start, end, step, callback) {
  if (isNullish(end)) {
    end = start;
    start = 0;
  }
  step ?? (step = 1);
  if (end < start && step > 0 || start > end && step < 0) [start, end] = [end, start];
  const num = Math.floor((end - start) / step);
  const result = linspace(start, start + step * (num - 1), num, callback);
  result.push(end);
  return result;
}
function linspace(start, end, num = 1e3, callback) {
...
const erf = /* @__PURE__ */ (() => {
  ...
  return (val) => {
    ...
    for (let i = 0; i < 5; i++) {
      prod *= (square + c2[i] * abs + c1[i]) / (square + b2[i] * abs + b1[i]);
    }
...
const lanczosG8N12 = {
  ...
  coeffs_: [
    ...
    -6.253671612368916,
    0.03464276245473681,
    -7477617197444298e-22,
    630412538218523e-22,
    -27405717035683877e-24,
    4048694881756761e-24
  ],
  /**
...
const gamma = (a) => {
  ...
  if (a < 0.5)
    return Math.PI / (Math.sin(Math.PI * a) * gamma(1 - a));
  const g = lanczosG8N12.g;
  const factors = [
    (a - 0.5) * Math.log(a + g - 0.5),
    0.5 - a - g,
...
const beta = (() => {
  const C = SQRT2PI * Math.exp(0.5 - lanczosG8N12.g);
  return (a, b) => {
    if (isNaN(a + b) || a <= 0 || b <= 0) return NaN;
    if (a === 1) return 1 / b;
    if (b === 1) return 1 / a;
    if (a < 0.5) return Math.PI / (b * Math.sin(Math.PI * a) * beta(1 - a, a + b));
    const g = lanczosG8N12.g, s = lanczosG8N12.sum_(a) * (lanczosG8N12.sum_(b) / lanczosG8N12.sum_(a + b)), c = a + b + g - 0.5;
    a -= 0.5;
    b -= 0.5;
    return C * ((a + g) / c) ** a * ((b + g) / c) ** b / Math.sqrt(c) * s;
  };
})();
const betainc = /* @__PURE__ */ (() => {
  const getCoeff = (x, a, b, idx) => {
    let m;
    if (idx % 2) {
      m = (idx - 1) / 2;
      return -(a + m) * (a + b + m) * x / ((a + idx - 1) * (a + idx));
    } else {
      m = idx / 2;
      return m * (b - m) * x / ((a + idx - 1) * (a + idx));
    }
  };
  return function betainc2(x, a, b, regularized = true) {
    if (a <= 0 || b <= 0) return NaN;
    if (Number.isNaN(x) || x < 0 || x > 1) return NaN;
    if (x === 0) return 0;
    if (x === 1) return regularized ? 1 : beta(a, b);
    let complement = false, scale = regularized ? beta(a, b) : 1, numeratorM2 = 0, numeratorM1 = 1, denominatorM2 = 1, denominatorM1 = 1, idx = 1, coeff, numerator, denominator, resultM1, result = Infinity;
    if (x > (a + 1) / (a + b + 2)) {
      complement = true;
...
export {
  ...
  SQRTPI_2 as S,
  SQRT2PI as T,
  storeArgsInitializer as U,
  VScaleTransition as V,
  PLACE as W,
  binomialCoeff as X,
  sum as Y,
  arange as a,
  ...
  standardEasing as e,
  forwardRefs as f,
  acceleratedEasing as g,
  getTargetBox as h,
  isNullish as i,
  VSlideYTransition as j,
  makeVSlideGroupProps as k,
  linspace as l,
  mergeObjects as m,
  ...
  toCSSLength as t,
  useScopeId as u,
  vuetifyInputKeydownWrapper as v,
  makeGroupItemProps as w,
  useGroupItem as x,
  VExpandXTransition as y,
  useNestedGroupActivator as z
};
